...
// ── Tab navigation ────────────────────────────────────────────
const tabs    = { menu: null, stock: null, users: null };
let activeTab = 'menu';
...
function renderMenuTable(items) {
  ...
  if (!items.length) {
    tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--text-muted);padding:2rem">Tidak ada menu</td></tr>';
    return;
  }
  tbody.innerHTML = items.map(item => `
    <tr>
      ...
      <td>
        <span class="${item.stock <= 5 ? 'stock-low' : item.stock <= 10 ? 'stock-mid' : 'stock-ok'} item-stock">
          ${item.stock}
        </span>
      </td>
...
async function openMenuModal(id) {
  ...
  if (id) {
    ...
    if (item) {
      ...
      if (item.image) {
        document.getElementById('mPreview').src = item.image;
        document.getElementById('mPreviewWrap').style.display = '';
      }
    }
  }
...
document.getElementById('menuModalCancel').addEventListener('click', () => {
...
document.getElementById('menuForm').addEventListener('submit', async (e) => {
  ...
  try {
    if (editingMenuId) {
      await API.updateMenu(editingMenuId, data);
    } else {
...
async function deleteMenu(id, name) {
  Modal.confirm('🗑️', 'Hapus Menu', `Hapus menu "${name}"? Data tidak bisa dikembalikan.`, async () => {
    try {
...
function renderStockTable(items) {
  ...
  if (!items.length) {
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:2rem">Tidak ada menu</td></tr>';
    return;
  }
  tbody.innerHTML = items.map(item => `
    <tr>
...
async function saveStock(id) {
  ...
  if (isNaN(stock) || stock < 0) {
    Modal.alert('⚠️', 'Input Tidak Valid', 'Stok harus angka >= 0');
    return;
  }
  try {
    ...
    if (span) {
      ...
      span.className = `${stock <= 5 ? 'stock-low' : stock <= 10 ? 'stock-mid' : 'stock-ok'} item-stock`;
    }
...
function renderUsersTable(users) {
  ...
  if (!users.length) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:2rem">Tidak ada user</td></tr>';
    return;
  }
...
document.getElementById('userForm').addEventListener('submit', async (e) => {
  ...
  try {
    if (editingUserId) {
      ...
      if (!password) { Modal.alert('⚠️', 'Password Wajib', 'Password wajib diisi untuk user baru.'); return; }
...
async function deleteUser(id, username) {
  if (id === session.id) {
    Modal.alert('⚠️', 'Tidak Bisa', 'Tidak bisa menghapus akun yang sedang login.');
    return;
  }
