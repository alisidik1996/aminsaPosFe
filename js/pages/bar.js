  ...
  if (!orders.length) {
    grid.innerHTML = `
      <div class="empty-state">
        ...
        <div>Tidak ada pesanan di bar saat ini.</div>
        <div style="font-size:.9rem;color:#1d4ed8;margin-top:.5rem">Tunggu pesanan dari kasir...</div>
      </div>
    `;
    return;
  }
...
async function loadData() {
  try {
    allItems = await API.getStationItems('bar');
    renderOrders();
  } catch (err) {
    ...
    if (allItems.length === 0) {
      document.getElementById('ordersGrid').innerHTML = `
        <div class="empty-state">
          ...
          <div style="font-size:.9rem;color:#1d4ed8;margin-top:.5rem">Pastikan backend berjalan di localhost:3000</div>
        </div>
      `;
    }
  }
